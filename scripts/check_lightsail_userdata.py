...
TEMPLATE = Path(__file__).resolve().parent.parent / "aws" / "lightsail-template.yaml"
...
def user_data_script(text: str) -> str:
    ...
    for i, line in enumerate(lines):
        ...
        sys.exit("check: no UserData in " + str(TEMPLATE))
    body, indent = [], None
...
def main() -> int:
    ...
    for shell in ("dash", "sh"):
        ...
        if done.returncode != 0:
            ...
            return 1
        print(f"OK: {len(prefix)} lines parse under {shell}; guard on line {guard_at + 1}.")
        return 0
