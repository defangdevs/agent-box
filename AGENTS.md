...
- **`modules/src/default-agents.md`** (plus `default-agents-webhook.md` for
  ...
  `users.<name>.agentsMd` (e.g. `deploy/aws/template.yaml`'s `AgentsMd` parameter).
...
- `cfn-lint deploy/aws/template.yaml deploy/aws/lightsail-template.yaml` validates the CloudFormation templates.
- `python3 scripts/check_azure_template.py` validates the Azure deployment: that `deploy/azure/agent-box.json` still matches a fresh `bicep build` of `deploy/azure/agent-box.bicep` (the JSON is a build artifact and is what the README's Deploy button serves, so an edit that never reached it ships nothing), that the bootstrap the template carries renders with no placeholder left, stays ASCII and parses under `bash -n`, and that the web password never leaves the extension's `protectedSettings` for the API-readable `settings`. Pass `--no-build` to skip the recompile on a machine with no Bicep CLI. Recompile with `cd deploy/azure && az bicep build --file agent-box.bicep`.
...
- **No nix on the box? Two of the three committed artefacts are still
  reachable.** `nix run .#assemble` is exactly
  `python3 bin/assemble-module.py`, and `python3 tests/test_agentbox.py
  --update` needs nothing but python - so the generated module and
  `tests/native/expected/` can be regenerated anywhere. `nix run
  .#update-golden` cannot. The golden SETTINGS payload is still mechanically
  reproducible: `tests/golden/web/payloads/agent-box-settings/bin/agent-box-settings`
  is its own first line (the normalized shebang), then
  `modules/src/lib/envstore.py`, then two newlines, then
  `python3 bin/assemble-module.py --resolve modules/src/settings-daemon.py`
  - the composition flake.nix's `runtime-profile` check spells out. Rebuild
  the COMMITTED file that way first and diff it to prove the recipe, then
  apply it to your change. Everything else under `tests/golden/` waits for
  CI, so say in the PR which fixtures you regenerated by hand and let
  `golden-snapshot` be the authority.
...
History uses concise imperative subjects and scoped Conventional Commit forms such as `feat(web): ...`, `fix(sessions): ...`, and `docs(agents-md): ...`. Reference issues when applicable. Pull requests should explain motivation, summarize user-visible and security effects, list exact checks run, and link the issue when there is one - a PR that IS the fix, filed instead of an issue, has none to link. Include screenshots for changes to the workspace or settings UI, and call out AWS cost, IAM, networking, or migration impacts.
