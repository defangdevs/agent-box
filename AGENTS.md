...
- `nix build -L .#checks.<system>.env-file-format` runs `tests/env-file-format.py`: the per-user secrets file is parsed by the settings daemon (Python) and by the spawn wrapper / session CLI (shell), and this asserts the two agree on every shape a value can take — including the quoted, multi-line one a PEM needs.
