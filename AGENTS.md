...
- `nix build -L .#checks.<system>.env-file-format` runs `tests/env-file-format.py`: every shape a stored secret can take — including the quoted, multi-line one a PEM needs — is written and read back through `modules/src/env-file.py` and through the CLI the shell callers use. A value that does not survive its own encoding comes back truncated rather than raising, so nothing else would catch it.
- `nix build -L .#checks.x86_64-linux.<name>` runs an individual VM test such as `sessions` or `settings-page`.

The eval-level checks (`multi-user`, `module-single-file`, `download-route`, `webhook-route`, `module-generated-up-to-date`, `env-file-format`) and `nix run .#assemble` are exposed for both `x86_64-linux` and `aarch64-linux`, so they run natively on a Graviton box like the deployed fleet. The qcow2 image and the interactive `runNixOSTest` checks are `x86_64-linux`-only: a NixOS test needs a same-arch KVM guest, so building them from another arch would fall back to unusably slow TCG. Widen `vmSystems` in `flake.nix` to offer them elsewhere.
