...
Attach those screenshots with the `uploads.github.com/user-attachments/assets` endpoint documented in the shipped guide (`modules/src/default-agents.md`, "Putting a screenshot in a GitHub issue or PR") — the same store the browser's paste uses. Do **not** add another `assets/*` branch to this repo: PRs #210, #228 and #243 each parked PNGs on one to keep binaries out of the diff, each said "delete after merge", and all four branches (`assets/175-new-session-ui`, `assets/227-webhook-subscriptions-panel`, `assets/236-long-session-names`, `assets/241-stopped-session-ui`) are still here. Leave them, and leave the `defangdevs/assets` repo — closed PRs #133 and #209 embed images from them, and deleting either breaks that history — but do not add to either.
