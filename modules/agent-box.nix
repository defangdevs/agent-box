...
let
  ...
  defaultAgentsMd = ''
    ...
    - Secrets go in ~/.config/agent-box/env (KEY=value, one per line). Set them
      with `agent-box-session env set KEY VALUE` (or `env ls` / `env rm KEY`, or
      the settings page); they load on the next session (re)start, eg. GH_TOKEN
      is read automatically, so `git clone https://github.com/...` just works.
              ...
              <form method="post" action="{action_base}/sessions/add">
                ...
                <div class="row">
                  <textarea name="prompt" rows="2"
                            placeholder="kickoff prompt (optional) — the task to start on; a respawn resumes it"></textarea>
                </div>
              </form>
            </div>
