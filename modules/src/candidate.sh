...
#
# Its `die` only exits the COMMAND SUBSTITUTION it runs in, so both callers
# must check the status: with `set -u` alone (and no `set -e`, deliberately
# — every refusal here is a `die` with a reason, not a bare non-zero), an
# unchecked `head=$(tracked_head)` would carry on with head='', print an
# empty tracked rev from --status, and let the "is it the tracked head
# already" test below pass by comparing against nothing.
tracked_head() {
  b=${SRC_BRANCH:-}
  if [ -n "$b" ]; then
...
case "${1:-}" in
  ...
  (--status)
    ...
    elif ! command git -C "$SRC_DIR" cat-file -e "$head^{commit}" 2>/dev/null; then
      # The remote has moved on and the tree has not fetched it yet, which is
      # the ordinary state of a box between updates. merge-base would exit
      # 128 on the missing object and fall through to the warning below,
      # reporting a perfectly normal box as one the guard has a problem with.
      # Say what is actually known instead: there is a newer rev, and this
      # tree cannot place itself against it without a fetch — which --status,
      # being a question, will not do.
      printf 'state:   an update is available (%.12s); this tree has not fetched it, so it cannot say more\n' "$head"
