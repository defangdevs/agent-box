...
def capacity_check(sessions, targets=(), spawning=False, live=None, limit=None):
    ...
    pending = {name for name, entry in sessions.items()
               ...
               if entry.get("stopped") is not True
               if entry.get("died") is None}
    used = live | pending
    targets = set(targets)
    if spawning:
