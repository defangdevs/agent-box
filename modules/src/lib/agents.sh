...
agent_upgrade() {
  ...
  # Read the pin only NOW, holding the lock: two upgrades for the same
  # agent can both reach this function before either acquires it (the
  # update service loops over every user; a person can run this by hand
  # at the same moment). Reading it earlier let the second process act on
  # a pin the first had already moved past - if the second's own `nix
  # profile add` then failed, its recovery path restored what it still
  # believed was "the previous pin", silently undoing the first process's
  # already-successful upgrade.
