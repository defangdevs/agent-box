...
#
# 1 once the "could not bring it up" warning below has been printed, so a
...
ensure_tmux_server() {
  ...
  if $TMUX start-server \; set-option -g exit-empty off 2>/dev/null; then
    _tmux_boot_warned=0
  elif [ "$_tmux_boot_warned" != 1 ]; then
    # A retry forever with zero signal would recreate the exact symptom
    # this fix exists to remove (silently blocked connect cards) with no
    # clue pointing at the cause, so say it once per streak rather than
    # trusting the retry alone.
    echo "supervisor: could not bring up the tmux server; connect cards" \
         "stay blocked until this recovers, retrying every tick" >&2
    _tmux_boot_warned=1
  fi
}
ensure_tmux_server
...
seed_claude_state() {
  ...
  # `git -C` changes directories but does not override inherited repository
  # selectors. Trust must follow the registered cwd, not a GIT_* setting from
  # the supervisor environment that points at some other repository.
  claude_git_dir="$(env -u GIT_DIR -u GIT_COMMON_DIR -u GIT_WORK_TREE \
    -u GIT_OBJECT_DIRECTORY \
    git -C "$1" rev-parse --path-format=absolute --git-dir 2>/dev/null)" \
    || claude_git_dir=
  claude_git_common_dir="$(env -u GIT_DIR -u GIT_COMMON_DIR -u GIT_WORK_TREE \
    -u GIT_OBJECT_DIRECTORY \
...
start_session() {
  ...
  # One session kind opts out of the parking branch:
  ...
  # shell — the pane IS a login shell, so there is no separate agent whose
  # crash would be worth a nested post-mortem bash; whatever code it exits
  # with, typing `exit` (or Ctrl-D) is always the user asking to be done, not
  # a crash to self-heal over (issue #678). `;` runs the mark unconditionally,
  # unlike the `&&`/`||` pair above, and it is always told status 0 so the
  # session parks (stopped=true) instead of a spurious `died` — matching
  # `/quit` on every other harness: park, don't respawn, and `agent-box-session
  # restart` brings a fresh shell back on request.
  [ "$agent" = shell ] && epilogue=" ; $_mark $_qname 0"
