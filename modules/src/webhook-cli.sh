...
Two delivery shapes:
  ...
  --deliver-to subagent  standing watch, for events no session owns (new
                         ...
                         refusal, and `ls` says so too once the box is at the
                         ceiling.
...
# box inert and it read like a quiet week (issue #170). These three read the
# state the wrapper decides on, so `status` and `ls` can say it.
#
# Since #301 the refusal is a DEFERRAL (the wrapper exits 75), so the batch
# waits for a slot instead of dying at the cap — but the cap is still what an
  ...
  subscribe)
    ...
    if [ "${1:-}" != "-h" ] && [ "${1:-}" != "--help" ]; then
      ...
      if [ "$have_events" = 1 ]; then
        ...
        # Parsed HERE, not left to the jq that composes the AND below.
        # That jq's exit status cannot be seen through the `set -- "$@"` it
        # feeds, so a malformed body used to compose to an EMPTY --include
        # and exit 0 — the worst answer this whole file can give, since a
        # filter matching nothing reads as a successful subscription and
