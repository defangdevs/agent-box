...
| `restartNotice` | `"auto"` | Whether a respawned claude session gets the built-in "you were interrupted and automatically restarted" prompt. `--resume` restores the transcript but starts no turn, so a session with outstanding work waits forever without one (issue #548); sending one revives the whole transcript, which is not free. `"auto"` sends it only to a session that has something outstanding - an unresolved hook lease, a webhook subscription filter, or a transcript cut mid-turn. `"always"` sends it on every respawn whose transcript holds real work; `"never"` never does. A per-session `resumePrompt` still overrides all three. |
...
- **A second, opt-in exception: `/<user>/auth/handoff`.** Off unless a box is
  ...
  correctly signed token for an account this box does not host is refused
  (`web.portalProject` optionally narrows this to one project within it). Basic auth stays an
