...
{ agent-box }:
{
  ...
  testScript = ''
    ...
    # initialPrompt (first-spawn only, cleared on resume like the CLI).
    client.succeed(
        f"{curl} -u agent:testpassword -o /dev/null -w '%{{http_code}}' "
        "-d 'name=webp&agent=claude' --data-urlencode 'prompt=hello there' "
        "https://box.test/sessions/add | grep -x 303"
    )
    machine.succeed(
        "jq -e '.sessions.webp.initialPrompt == \"hello there\"' "
        "/home/agent/.config/agent-box/sessions.json"
    )
    client.succeed(
        f"{curl} -u agent:testpassword -o /dev/null -w '%{{http_code}}' "
        "-d 'name=webp' https://box.test/sessions/delete | grep -x 303"
    )
